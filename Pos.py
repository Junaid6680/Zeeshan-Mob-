...
def create_pdf(bill_id, customer, phone, items, total, paid):
    ...
    pdf.set_text_color(240, 240, 240) # Boht halka grey color
    # Diagonal text as watermark
    pdf.rotate(45, 100, 150)
    pdf.text(30, 190, "ZEESHAN MOBILE ACCESSORIES")
    pdf.rotate(0) # Rotation wapas seedhi karne ke liye
    ...
    # Left Side: Bill Info | Right Side: Customer Info
    pdf.set_font("Arial", "B", 11)
    curr_date = datetime.now().strftime('%d-%m-%Y %H:%M')
    pdf.cell(95, 7, f"Bill No: {bill_id}", ln=0) 
    pdf.cell(95, 7, f"Customer: {customer}", ln=1, align="R") 
    
    pdf.set_font("Arial", size=11)
    pdf.cell(95, 7, f"Date: {curr_date}", ln=0) 
    pdf.cell(95, 7, f"Phone: {phone}", ln=1, align="R") 
    pdf.ln(10)
...
with c2:
    ...
    if st.button("✅ Generate & Save Bill"):
        if st.session_state.cart:
            ...
            pdf_bytes = create_pdf(next_bill_id, final_cust, final_phone, st.session_state.cart, total_bill, amt_paid)
