...
# Tables banana (Agar pehle se nahi bani)
...
# ================= PDF GENERATION FUNCTION (Fixed Error) =================
def create_pdf(bill_id, customer, phone, items, total, paid):
    ...
    # Output as bytes for Streamlit download button
...
with c1:
    ...
    # Phone number fetch karna
    final_phone = ""
    if final_cust != "Walk-in":
        final_phone = cust_data[cust_data.name == final_cust].iloc[0]["phone"]
        st.caption(f"Phone: {final_phone}")

    with st.expander("➕ Add New Customer to Database"):
        ...
        if st.button("Save Customer"):
            if n_name:
                try:
                    ...
                    st.error("Ye naam pehle se mojud hai.")
...
with c2:
    ...
    if st.button("✅ Generate & Save Bill"):
        if not st.session_state.cart:
            st.warning("Cart khali hai, pehle koi item add karein.")
        else:
            b_id = f"BILL-{datetime.now().strftime('%y%m%d%H%M%S')}"
            
            # Database mein sale save karna
            ...
            st.download_button(
                ...
                data=pdf_bytes,
                file_name=f"Zeeshan_Mobile_{b_id}.pdf",
                mime="application/pdf"
            )
            st.success(f"Sale recorded! Bill ID: {b_id}")
            # Note: Main cart clear nahi kar raha taake user pehle download kar le.
            # Agar auto-clear chahiye to st.session_state.cart = [] add kar dein.
