...
with c2:
    ...
    if st.button("✅ Generate & Save Bill"):
        if st.session_state.cart:
            ...
            st.session_state.cart = [] # Cart clear after sale
...
# ================= PAYMENT RECEIVING SECTION (FIXED) =================
st.divider()
st.subheader("💸 Receive Old Payment (Recovery)")

# Database se ledger nikalna taake calculation sahi ho
ledger_query = """
SELECT customer, 
...
# Sirf un customers ki list jin ka udhaar baaki hai
debtor_customers = ledger_df[ledger_df['Remaining_Balance'] > 0]['customer'].tolist()

with st.expander("Record Cash Recovery (Purana Udhaar Wapsi)"):
    if debtor_customers:
        rcv_col1, rcv_col2 = st.columns(2)
        with rcv_col1:
            pay_cust = st.selectbox("Select Customer for Recovery", debtor_customers)
            # Current balance dikhana
            current_bal = ledger_df[ledger_df['customer'] == pay_cust]['Remaining_Balance'].values[0]
            st.info(f"Current Balance for {pay_cust}: Rs. {current_bal}")
        with rcv_col2:
            rcv_amount = st.number_input("Amount Received", min_value=0.0, max_value=float(current_bal), step=10.0)
        
        if st.button("Submit Recovery Payment"):
            if rcv_amount > 0:
                # Recovery entry: Bill Total = 0, Paid = rcv_amount
                c.execute("INSERT INTO sales(date, customer, total, paid) VALUES (?,?,?,?)",
                          (datetime.now().strftime('%Y-%m-%d'), pay_cust, 0.0, rcv_amount))
                conn.commit()
                st.success(f"Success! Rs. {rcv_amount} recovered from {pay_cust}.")
                st.rerun()
    else:
        st.write("No outstanding balances found.")

# ================= CUSTOMER LEDGER DISPLAY =================
st.subheader("📒 Customer Ledger (Total Dues)")
st.dataframe(ledger_df[ledger_df['Remaining_Balance'] > 0], use_container_width=True)

# ================= RECENT HISTORY =================
st.subheader("📜 Recent Sales History")
history = pd.read_sql("SELECT id AS Bill_No, date, customer, total, paid, (total - paid) AS balance FROM sales ORDER BY id DESC", conn)
