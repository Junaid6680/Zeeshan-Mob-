...
if not st.session_state.sales_history.empty:
    h = st.session_state.sales_history.copy()
    h["Date"] = pd.to_datetime(h["Date"])

    today = pd.Timestamp.now().date()
    today_sale = h[h["Date"].dt.date == today]["Total"].sum()
    weekly_sale = h[h["Date"] >= (pd.Timestamp.now() - pd.Timedelta(days=7))]["Total"].sum()
    monthly_sale = h[h["Date"] >= (pd.Timestamp.now() - pd.Timedelta(days=30))]["Total"].sum()

    r1, r2, r3 = st.columns(3)
    r1.metric("Today Sale", f"Rs. {today_sale}")
    r2.metric("Weekly Sale", f"Rs. {weekly_sale}")
    r3.metric("Monthly Sale", f"Rs. {monthly_sale}")
